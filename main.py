...
def filter_bulk_tweets(tweets):
    ...
    Pure retweets are excluded - but quote tweets (user adds comment) ARE counted.
    """
    bulk_tweets = []
    ...
    for tweet in tweets:
        # Skip pure retweets - but ALLOW quote tweets (user adds their own comment)
        # Quote tweets have quoted_tweet/quoted_status - those are original content
        has_quote = tweet.get("quoted_tweet") or tweet.get("quoted_status") or tweet.get("is_quote_status")
        
        # Pure retweet checks (skip these)
        if not has_quote:
        ...
        # Always skip if text starts with "RT @" (pure retweet format)
